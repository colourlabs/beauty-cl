...
public:
  ...
  timer(const beauty::duration &d, Callback &&cb, bool repeat = false)
      : _app(beauty::application::Instance()), _timer(_app.ioc()),
        _duration(d) {
...
// --------------------------------------------------------------------------
template <typename Callback>
std::shared_ptr<beauty::timer> after(const duration &d, Callback &&cb,
                                     bool repeat = false) {
  auto timer =
      std::make_shared<beauty::timer>(d, std::forward<Callback>(cb), repeat);
  timer->start();
  return timer;
}

template <typename Callback>
std::shared_ptr<beauty::timer> after(double seconds, Callback &&cb,
                                     bool repeat = false) {
  return after(std::chrono::milliseconds((int)(seconds * 1000)),
               std::forward<Callback>(cb), repeat);
}

template <typename Callback>
std::shared_ptr<beauty::timer> repeat(const duration &d, Callback &&cb) {
  return after(d, std::forward<Callback>(cb), true);
}

template <typename Callback>
std::shared_ptr<beauty::timer> repeat(double seconds, Callback &&cb) {
