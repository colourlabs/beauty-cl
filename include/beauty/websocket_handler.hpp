...
using ws_on_receive_cb =
    std::function<void(const ws_context &, const char *, std::size_t, bool)>;
using ws_on_disconnect_cb = std::function<void(const ws_context &ctx)>;
using ws_on_error_cb =
    std::function<void(boost::system::error_code, const char *what)>;

// --------------------------------------------------------------------------
struct ws_handler {
  ws_on_connect_cb on_connect = [](const ws_context &) {};
  ws_on_receive_cb on_receive = [](const ws_context &, const char *,
                                   std::size_t, bool) {};
