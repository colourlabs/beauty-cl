...
public:
  explicit websocket_session(asio::ip::tcp::socket &&socket,
      ...
      : _websocket(std::move(socket)), _route(route) {}
  ...
  void run(const beauty::request &req) {
    ...
    _websocket.async_accept(
        req, [me = this->shared_from_this()](auto ec) { me->on_accept(ec); });
  }
...
private:
  void on_accept(beast::error_code ec) {
    ...
    // std::cout << "websocket_session: " << _ws_context.uuid << " - on accept"
    // << std::endl;
  ...
  void do_read() {
    // std::cout << "websocket_session: " << _ws_context.uuid << " - do read" <<
    // std::endl;
  ...
  void on_read(beast::error_code ec, std::size_t) {
    // std::cout << "websocket_session: " << _ws_context.uuid << " - on read" <<
    // std::endl;
  ...
  void on_write(beast::error_code ec, std::size_t) {
    // std::cout << "websocket_session: " << _ws_context.uuid << " - on write"
    // << std::endl;
    if (ec) {
