...
public:
  attribute() = default;
  explicit attribute(std::string str) : _value(std::move(str)) {}

  std::string as_string(const char *default_value = "") const noexcept {
    return as<std::string>(default_value);
  }

  int as_integer(int default_value = 0) const { return as<int>(default_value); }
...
private:
  template <typename Type> Type as(const Type &default_value = {}) const {
    if (_value.empty()) {
