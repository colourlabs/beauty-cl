...
public:
  websocket_client(asio::io_context &ioc, url &&url, ws_handler &&handler)
      : _resolver(asio::make_strand(ioc)), _websocket(asio::make_strand(ioc)),
        _url(std::move(url)), _ws_handler(std::move(handler)) {}
  ...
  void send(std::string &&msg, bool binary_mode = false) {
    beast::net::post(
        ...
        [me = this->shared_from_this(), str = std::move(msg),
         binary_mode]() mutable { me->do_write(std::move(str), binary_mode); });
  }
...
private:
  ...
  void on_connect(beast::error_code ec,
    ...
    _ws_context.remote_endpoint =
        _websocket.next_layer().socket().remote_endpoint();
    _ws_context.local_endpoint =
        _websocket.next_layer().socket().local_endpoint();
    _ws_context.uuid = make_uuid();
    _ws_context.target = _url.path();
    _ws_context.route_path = _url.path();
    //_ws_context.attributes = _url.query();
    // std::cout << "websocket_client: " << _ws_context.uuid << " - on connect"
    // << std::endl;
    ...
    _websocket.set_option(beast::websocket::stream_base::decorator(
        [](beast::websocket::response_type &res) {
          res.set(beast::http::field::server,
                  std::string(BEAUTY_PROJECT_VERSION) + " beauty websocket");
        }));
    ...
    _websocket.async_handshake(
        host, _url.path(),
        [me = this->shared_from_this()](auto ec) { me->on_handshake(ec); });
  }

  void on_handshake(beast::error_code ec) {
    // std::cout << "websocket_client: " << _ws_context.uuid << " - on
    // handshake" << std::endl;
    if (ec) {
  ...
  void do_read() {
    // std::cout << "websocket_client: " << _ws_context.uuid << " - do read" <<
    // std::endl;
    _websocket.async_read(
        _buffer, [me = this->shared_from_this()](auto ec, std::size_t size) {
          me->on_read(ec, size);
        });
  }

  void on_read(beast::error_code ec, std::size_t) {
    // std::cout << "websocket_client: " << _ws_context.uuid << " - on read" <<
    // std::endl;
    ...
    _ws_handler.on_receive(_ws_context,
                           static_cast<const char *>(_buffer.cdata().data()),
                           _buffer.size(), _websocket.got_text());
  ...
  void on_write(beast::error_code ec, std::size_t) {
    // std::cout << "websocket_client: " << _ws_context.uuid << " - on write" <<
    // std::endl;
    if (ec) {
