...
class session_client
    : public std::enable_shared_from_this<session_client<SSL>> {
public:
#if BEAUTY_ENABLE_OPENSSL
  using stream_type =
      std::conditional_t<SSL, asio::ssl::stream<asio::ip::tcp::socket>, void *>;
#else
...
private:
  ...
  struct request_context {
    explicit request_context(asio::io_context &ioc) : timer(ioc) {
    ...
    static std::shared_ptr<request_context>
    Create(asio::io_context &ioc, beauty::request &&req, const beauty::url &url,
           const beauty::duration &d,
           std::optional<client::client_cb> cb = {}) {
      ...
      req_ctx->request.target(std::string(url.path()) +
...
public:
  ...
  explicit session_client(asio::io_context &ioc)
      : _ioc(ioc), _resolver(_ioc), _socket(_ioc),
        _strand(asio::make_strand(ioc)) {}

#if BEAUTY_ENABLE_OPENSSL
  template <bool U = SSL, typename std::enable_if_t<U, int> = 0>
  session_client(asio::io_context &ioc, asio::ssl::context &ctx)
      : _ioc(ioc), _resolver(_ioc), _socket(_ioc), _stream(ioc, ctx),
        _strand(asio::make_strand(ioc)) {}
#endif
  ...
  void run(beauty::request &&req, const beauty::url &url,
           const beauty::duration &d) {
    // Create a request context to pass on each callback
  ...
  void run(beauty::request &&req, const beauty::url &url,
           const beauty::duration &d, client::client_cb &&cb) {
    // Create a request context to pass on each callback
    auto req_ctx =
        request_context::Create(_ioc, std::move(req), url, d, std::move(cb));
  ...
  void run(std::shared_ptr<request_context> req_ctx) {
    if (req_ctx->timer.expiry() != asio::steady_timer::time_point()) {
      req_ctx->timer.async_wait([me = this->shared_from_this(),
                                 req_ctx](const boost::system::error_code &ec) {
    ...
    if constexpr (SSL) {
      // std::cout << "session_client:" << __LINE__ << " : Connection required ?
      // " << _requests.empty() << " && " << !_stream.next_layer().is_open() <<
      // std::endl;
#if BEAUTY_ENABLE_OPENSSL
      connection_required =
          (_requests.empty() && !_stream.next_layer().is_open());
      ...
      // std::cout << "session_client:" << __LINE__ << " : Connection required ?
      // " << _requests.empty() << " && " << !_socket.is_open() << std::endl;
      connection_required = (_requests.empty() && !_socket.is_open());
    }
    // std::cout << "session_client:" << __LINE__ << " : Start a request, with
    // new connection: " << connection_required << std::endl;
    ...
    if (_pending_request) {
      // std::cout << "session_client:" << __LINE__ << " : There is a waiting
      // connection or read ack, nothing to do to rearm" << std::endl;
    } else {
      // std::cout << "session_client:" << __LINE__ << " : No waiting connection
      // ack or read ack, we must rearm" << std::endl;
      do_write();
    }
  }

  void do_resolve() {
    // std::cout << "session_client:" << __LINE__ << " : Try to resolve the
    // host" << std::endl;
    ...
    _resolver.async_resolve(
        (*_requests.begin())->url.host(), port_view,
  ...
  void on_resolve(const boost::system::error_code &ec,
                  const asio::ip::tcp::resolver::results_type &results) {
    // std::cout << "session_client:" << __LINE__ << " : on_resolve" <<
    // std::endl;
    if (ec) {
      std::lock_guard guard{_requests_mtx};
    ...
    if constexpr (SSL) {
#if BEAUTY_ENABLE_OPENSSL
      // std::cout << "session_client:" << __LINE__ << " : Try a ssl connection"
      // << std::endl;
      asio::async_connect(
          _stream.next_layer(), results.begin(), results.end(),
          [me = this->shared_from_this()](
              const boost::system::error_code &ec,
      ...
      // std::cout << "session_client:" << __LINE__ << " : Try a connection" <<
      // std::endl;
      asio::async_connect(
          _socket, results.begin(), results.end(),
          [me = this->shared_from_this()](
              const boost::system::error_code &ec,
#if BOOST_VERSION >= 108700
              const asio::ip::basic_resolver_iterator<asio::ip::tcp> &it
  ...
  void on_connect(const boost::system::error_code &ec) {
    // std::cout << "session_client:" << __LINE__ << " : on_connect" <<
    // std::endl;
    if (ec) {
      std::lock_guard guard{_requests_mtx};
  ...
  void do_write() {
    ...
    if (_requests.empty()) {
      // std::cout << "session_client:" << __LINE__ << " : ... nothing to do" <<
      // std::endl;
      _pending_request = false;
      return;
    }

    // std::cout << "session_client:" << __LINE__ << " :start a write" <<
    // std::endl;
    _pending_request = true;
    ...
    if constexpr (SSL) {
      beast::http::async_write(
          _stream, (*_requests.begin())->request,
      ...
      beast::http::async_write(
          _socket, (*_requests.begin())->request,
          asio::bind_executor(_strand, [me = this->shared_from_this()](
                                           boost::system::error_code ec,
                                           std::size_t bytes_transferred) {
            me->on_write(ec, bytes_transferred);
  ...
  void on_write(boost::system::error_code ec, std::size_t) {
    ...
    if constexpr (SSL) {
      beast::http::async_read(
          _stream, _buffer, (*_requests.begin())->response,
          asio::bind_executor(_strand, [me = this->shared_from_this()](
                                           boost::system::error_code ec,
                                           std::size_t bytes_transferred) {
            me->on_read(ec);
      ...
      beast::http::async_read(
          _socket, _buffer, (*_requests.begin())->response,
          asio::bind_executor(_strand, [me = this->shared_from_this()](
                                           boost::system::error_code ec,
                                           std::size_t bytes_transferred) {
            me->on_read(ec);
...
private:
  void fail(request_context &req_ctx, boost::system::error_code ec,
            const char *msg /* not used */) {
    if (req_ctx.cb) {
      // std::cout << "session_client:" << __LINE__ << " : !!! FAILED !!! " <<
      // ec << " with " << msg << std::endl;
      if (!req_ctx.too_late) {
        req_ctx.cb(ec, {});
        req_ctx.cb = nullptr;
      }
