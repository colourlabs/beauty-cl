...
namespace beauty {
// --------------------------------------------------------------------------
application::application() : _work(asio::make_work_guard(_ioc)) {
...
application::application(asio::io_context &ioc)
    : _ioc_external(&ioc), _work(asio::make_work_guard(*_ioc_external)) {
#if BEAUTY_ENABLE_OPENSSL
  _ssl_context.set_verify_mode(asio::ssl::verify_none);
#endif
}
...
application::application(certificates &&c)
    : _work(asio::make_work_guard(_ioc)), _certificates(std::move(c)) {
...
// --------------------------------------------------------------------------
application::~application() { stop(false); }

// --------------------------------------------------------------------------
void application::start(int concurrency) {
...
// --------------------------------------------------------------------------
void application::stop(bool reset) {
...
// --------------------------------------------------------------------------
void application::post(std::function<void()> fct) {
  boost::asio::post(ioc().get_executor(), std::move(fct));
}

// --------------------------------------------------------------------------
application &application::Instance() {
  std::call_once(flag, [] { g_application = std::make_unique<application>(); });
  return *g_application;
}

#if BEAUTY_ENABLE_OPENSSL
// --------------------------------------------------------------------------
void application::load_server_certificates() {
  if (!_certificates->password.empty()) {
    _ssl_context.set_password_callback(
        [pwd = _certificates->password](
  ...
  if (!_certificates->certificat_chain.empty()) {
    _ssl_context.use_certificate_chain(
        asio::buffer(_certificates->certificat_chain.data(),
                     _certificates->certificat_chain.size()));
  }

  if (!_certificates->private_key.empty()) {
    _ssl_context.use_private_key(
        asio::buffer(_certificates->private_key.data(),
                     _certificates->private_key.size()),
        asio::ssl::context::file_format::pem);
  }

  if (!_certificates->temporary_dh.empty()) {
    _ssl_context.use_tmp_dh(asio::buffer(_certificates->temporary_dh.data(),
                                         _certificates->temporary_dh.size()));
  }

  if (_certificates->certificat_chain.empty() ||
      _certificates->private_key.empty()) {
    _ssl_context.set_verify_mode(asio::ssl::verify_none);
  }
}

// --------------------------------------------------------------------------
application &application::Instance(certificates &&c) {
...
// --------------------------------------------------------------------------
void start(int concurrency) { application::Instance().start(concurrency); }

// --------------------------------------------------------------------------
void run() { application::Instance().run(); }

// --------------------------------------------------------------------------
void wait() { application::Instance().wait(); }

// --------------------------------------------------------------------------
void stop() { application::Instance().stop(); }
...
// --------------------------------------------------------------------------
bool is_started() { return application::Instance().is_started(); }
