...
signal_set::signal_set(signal_cb &&cb)
    : _app(beauty::application::Instance()), _signals(_app.ioc()), _cb(cb) {}

// --------------------------------------------------------------------------
signal_set &signal_set::add(int s) {
  _signals.add(s);
  return *this;
}
...
void signal_set::run() {
  ...
  _signals.async_wait(
      [me = shared_from_this()](const boost::system::error_code &ec, int s) {
        if (!ec) {
          me->_cb(s);
...
// --------------------------------------------------------------------------
void signal(int s, signal_cb &&cb) { signal({s}, std::move(cb)); }
