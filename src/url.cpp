...
url::url(std::string u) : _url(std::move(u)) {
  ...
  if (url_split.size() < 3 || url_split[1].size() || url_split[2].empty()) {
  ...
  if (host.size()) {
    ...
    if (host[0] == '[') {
      ...
      if (found == std::string::npos)
        throw std::runtime_error("Invalid URL format for IPv6 [" +
                                 std::string(host) + "]");
      ...
      if ((found = host.find(':', found + 1)) != std::string::npos) {
        ...
        if (_port_view.empty()) {
          throw std::runtime_error("Invalid port for IPv6 [" +
                                   std::string(host) + "]");
        }
      }
    } else {
  ...
  if (_port_view.size()) {
    auto [p, ec] = std::from_chars(&_port_view[0],
                                   &_port_view[0] + _port_view.size(), _port);
    if (ec != std::errc()) {
      throw std::runtime_error("Invalid port number " +
                               std::string(_port_view));
    }
  }
  ...
  if (url_split.size() >= 3) {
    ...
    std::size_t pos =
        url_split[0].size() + 1 + url_split[1].size() + 1 + url_split[2].size();
...
// --------------------------------------------------------------------------
std::string url::strip_login_password() const {
  return scheme() + "://" + host() +
         (port() ? ":" + std::to_string(port()) : "") + path();
}
